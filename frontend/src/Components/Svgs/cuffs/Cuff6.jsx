import React from "react";

const Cuff6 = ({ image }) => {
  return (
    <svg
      id="adc8a826-a73d-46ae-b74f-07dfff5a7a38"
      data-name="CuffDesign6"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="150 400 360 250"
    >
      <defs>
        <style>
          {`.a67517fa-77df-474a-8872-5774c0b971d7,
                                    .b36715f5-376c-4f04-aece-37397abf0c66 {
                                      fill: none;
                                    }

                                    .a67517fa-77df-474a-8872-5774c0b971d7,
                                    .af1bb800-cd30-484a-95f7-2a98dd0633eb,
                                    .b36715f5-376c-4f04-aece-37397abf0c66 {
                                      stroke: #000;
                                      stroke-width: 1%;
                                      stroke-miterlimit: 10;
                                    }

                                    .b709450e-cf50-4d0a-bf0b-726ab2fb3034 {
                                      clip-path: url(#a24037a9-c143-48c7-971c-f2b80736290d);
                                    }

                                    .a67517fa-77df-474a-8872-5774c0b971d7 {
                                      stroke-width: 2px;
                                    }

                                    .af1bb800-cd30-484a-95f7-2a98dd0633eb {
                                      fill: #fff;
                                    }`}
        </style>
        <clipPath
          id="a24037a9-c143-48c7-971c-f2b80736290d"
          transform="translate(156.81 397.14)"
        >
          <path
            className="b36715f5-376c-4f04-aece-37397abf0c66"
            d="M332,43.47V200.9c-33.21-12.45-73.57-17.61-114.21-18.75L196.5,156.67c-22.75,13.46-39.21,22.47-47.51,26.91A907.94,907.94,0,0,0,17.11,201.87l.07-158.4A501.14,501.14,0,0,1,217.54,19.85C287.24,25.44,332,43.47,332,43.47Z"
          />
        </clipPath>
      </defs>
      <g className="b709450e-cf50-4d0a-bf0b-726ab2fb3034">
        <image
          width="768"
          height="1024"
          transform="translate(0) scale(1.8)"
          xlinkHref={image}
        />
      </g>
      <path
        className="b36715f5-376c-4f04-aece-37397abf0c66"
        d="M332,43.47V200.9c-33.21-12.45-73.57-17.61-114.21-18.75L196.5,156.67c-22.75,13.46-39.21,22.47-47.51,26.91A907.94,907.94,0,0,0,17.11,201.87l.07-158.4A501.14,501.14,0,0,1,217.54,19.85C287.24,25.44,332,43.47,332,43.47Z"
        transform="translate(156.81 397.14)"
      />
      <path
        className="a67517fa-77df-474a-8872-5774c0b971d7"
        d="M217.54,18.85v124.8c-7.44,4.72-14.6,9-21,13"
        transform="translate(156.81 397.14)"
      />
      <path
        className="b36715f5-376c-4f04-aece-37397abf0c66"
        d="M149,183.58"
        transform="translate(156.81 397.14)"
      />
      <circle
        className="af1bb800-cd30-484a-95f7-2a98dd0633eb btnColorChange"
        cx="319.92"
        cy="466.07"
        r="21.47"
      />
      <circle
        className="af1bb800-cd30-484a-95f7-2a98dd0633eb btnColorChange"
        cx="319.92"
        cy="519.07"
        r="21.47"
      />
      <path
        className="b36715f5-376c-4f04-aece-37397abf0c66"
        d="M196.5,156.67"
        transform="translate(156.81 397.14)"
      />
      <path
        className="af1bb800-cd30-484a-95f7-2a98dd0633eb"
        d="M332.05,201.69c-183,67-315,0-315,0,23-9,132-18,132-18l47-27,22,25C285.05,182.69,332.05,201.69,332.05,201.69Z"
        transform="translate(156.81 397.14)"
      />
    </svg>
  );
};

export default Cuff6;
